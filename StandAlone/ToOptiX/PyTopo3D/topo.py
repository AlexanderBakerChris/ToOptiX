...
from GeometricalSettings import *
...
def topo_start3d(volfrac, penal, rmin, matSets, lastStepIter, numbIterAfter,
    ...
    #changeMaterialModelIsActive = True

    if ThermalIsActive:
        [nodeDic, elemDic] = import_elements_nodes_as_dic(iFilePathT)
    elif StructIsActive:
        [nodeDic, elemDic] = import_elements_nodes_as_dic(iFilePathS)
    else:
        print "Wrong input"
    filterVal = None
    if filterIsActive:
    ...
    while iteration <= iterEndAnaOpti + numbIterAfter:
        ...
        if simpIsActive:
            print("SIMP is selected")
        else:
            print("BESO is selected")
    ...
    if mExpoInpIsActive:
        inpMeshFile = ExportInpMesh("ResultFEMesh.inp", elemDic)
